...
import com.example.RemotePatientMonitoringSystem02.repository.agriculture.CropDiseaseDetectionRepository;
...
public class CropDiseaseDetectionService {
    private final CropDiseaseDetectionRepository cropDiseaseDetectionRepository;

    public CropDiseaseDetectionService(CropDiseaseDetectionRepository cropDiseaseDetectionRepository) {
        this.cropDiseaseDetectionRepository = cropDiseaseDetectionRepository;
    }

    public List<CropDiseaseDetectionDevices> getAllDevices(){
        return cropDiseaseDetectionRepository.findAll();
    }

    public BigDecimal calculateTotalInvestment(List<CropDiseaseDetectionDevices> devices, List<Integer> quantities, int investmentPeriod) {
        BigDecimal totalInvestment = BigDecimal.ZERO;
        for (int i = 0; i < devices.size(); i++) {
            CropDiseaseDetectionDevices device = devices.get(i);
    ...
    public BigDecimal calculateTotalNetBenefit(List<CropDiseaseDetectionDevices> devices, List<Integer> quantities, int investmentPeriod) {
