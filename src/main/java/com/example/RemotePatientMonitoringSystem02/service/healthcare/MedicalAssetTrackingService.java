...
@Service
public class MedicalAssetTrackingService {

    private final MedicalAssetTrackingRepository medicalAssetTrackingRepository;

    public MedicalAssetTrackingService(MedicalAssetTrackingRepository medicalAssetTrackingRepository) {
        this.medicalAssetTrackingRepository = medicalAssetTrackingRepository;
    }

    public List<MedicalAssetTrackingDevices> getAllDevices(){
        return medicalAssetTrackingRepository.findAll();
    }

    public BigDecimal calculateTotalInvestment(List<MedicalAssetTrackingDevices> devices, List<Integer> quantities, int investmentPeriod) {
        BigDecimal totalInvestment = BigDecimal.ZERO;
        for (int i = 0; i < devices.size(); i++) {
            MedicalAssetTrackingDevices device = devices.get(i);
    ...
    public BigDecimal calculateTotalNetBenefit(List<MedicalAssetTrackingDevices> devices, List<Integer> quantities, int investmentPeriod) {
