...
import com.example.RemotePatientMonitoringSystem02.repository.realEstate.SmartHomeRepository;
...
public class SmartHomeService {
    ...
    public List<SmartHomeDevices> getAllDevices(){
        return smartHomeRepository.findAll();
    }

    public BigDecimal calculateTotalInvestment(List<SmartHomeDevices> devices, List<Integer> quantities, int investmentPeriod) {
        BigDecimal totalInvestment = BigDecimal.ZERO;
        for (int i = 0; i < devices.size(); i++) {
            SmartHomeDevices device = devices.get(i);
    ...
    public BigDecimal calculateTotalNetBenefit(List<SmartHomeDevices> devices, List<Integer> quantities, int investmentPeriod) {
